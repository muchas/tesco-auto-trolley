...
function getIngredientsList(apiKey: String, projectName: String) {
    const response = $.ajax({
        type: "POST",
...
function addToTrolley(productId: String, quantity: String, unitType: String) {
  const csrfToken = $('html').data('csrf-token');
  const response = $.ajax({
    type: "PUT",
...
function importProducts(apiKey: String, projectName: String, aliases) {
